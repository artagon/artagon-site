...
- Content collections: initially added at `src/content/config.ts` + `src/content/pages/vision.mdx`. Astro v6 migration moved the config to `src/content.config.ts` (loader API, `glob({ pattern, base })`) and switched the consumer to `getEntry` + standalone `render(entry)` from `astro:content`. `sharp` added to `package.json`.

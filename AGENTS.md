...
- `npm run build` — runs `prebuild` (sync) → `astro build` → `postbuild` (10-step chain: verify-prereqs → verify-design-prereqs → lint-tokens → font-self-hosting → SRI → CSP → skip-link → taglines → design.md → design-md-uniqueness; canonical order in `package.json:scripts.postbuild`, gated by `tests/lint-readme-postbuild-chain-sync.test.mts`).
...
- **DESIGN.md token assignments** — [`DESIGN.md`](./DESIGN.md) §lines 242-259 (semantic-alias prose + the `--ok`/`--bad`/`--accent` table) + §6.5 specify `--ok` for PERMIT/pass states, `--bad` for DENY/fail, `--accent` (= retained alias `--brand-teal`; canonical name today is `--accent` per pt86) reserved for primary CTAs and the trust-chain's CURRENT/EVALUATING stage. Components MUST consume the semantic alias, not the staging `--nd-*` token directly.

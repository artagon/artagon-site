...
- [ ] 5.6 Install + configure remark/rehype MDX-allowlist plugin enforcing `['StandardChip', 'StandardsRow', 'TrustChain', 'Diagram', 'Callout']`. Wire into `astro.config.mjs` `markdown.remarkPlugins`. Acceptance: a fixture MDX with `<UntrustedThirdPartyWidget>` MUST cause `astro build` to exit non-zero before rendering. Astro's built-in `components` config does NOT enforce — verified by sub-agent review.

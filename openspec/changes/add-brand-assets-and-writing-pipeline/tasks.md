...
- [ ] 5.4 Augment `src/content.config.ts`: extend the `writing` collection (pre-pt415 cited as `pages/writing` — collection is named `writing` per `src/content.config.ts:128`; sister to pt414 path-nesting fixes) with a second glob loader pointing at `.cache/content-repo/posts/**/*.{md,mdx}`, ALONGSIDE the existing local glob. Both feed the same Zod schema. Detect duplicate ids across the two sources and fail the build.

...
- Add `node:test` tests for `measure-font-payload.mjs`, `verify-font-metrics.mjs`, and `verify-font-subset-coverage.mjs` (mkdtemp + synthetic `.build/dist/` fixtures), following the flat `tests/*.test.mjs` layout.

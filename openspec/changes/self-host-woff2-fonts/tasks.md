...
- [ ] 1.3 Subset each binary to the declared `unicode-range` (Latin + Latin-Extended + numeric + ligatures + select symbols). Files: `public/assets/fonts/<family>/<weight>-<style>.woff2`. Acceptance: each individual WOFF2 binary is ≤ 60 KB after subsetting (per-file cap; the per-family-per-route budget is separately enforced in Phase 5 by `measure-font-payload.mjs`).
...
- [ ] 4.1 Modify `src/layouts/BaseLayout.astro` to accept `Astro.props.preloadFont?: string` (default: `"space-grotesk/500-normal"`, using `"<family>/<weight>-<style>"` format per D3/D7). Emit between 1 and 2 `<link rel="preload" as="font" type="font/woff2" crossorigin>` tags in `<head>` per the spec (marketing routes: exactly one; long-form routes: 1–2). Files: `src/layouts/BaseLayout.astro`. Acceptance: built `dist/index.html` contains 1 preload-link; built writing route contains 1–2 preload-links.
- [ ] 4.2 Audit each `src/pages/**/*.astro` route's BaseLayout invocation. Marketing routes use the default (`"space-grotesk/500-normal"`). Long-form `/writing/*` routes pass `preloadFont="fraunces/400-italic"`. Files: `src/pages/writing/[slug].astro` and any other long-form route. Acceptance: every route emits the right preload-link per its content type.

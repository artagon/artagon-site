# Tasks — self-host-woff2-fonts

> Phased implementation plan for the `self-host-woff2-fonts` OpenSpec change. Phases land in order; tasks within a phase MAY parallelize. Each task lists files touched + a verifiable acceptance signal. See `proposal.md` for "Why," `design.md` for "How," `specs/<capability>/spec.md` for binding requirements.
...
- [ ] 1.3 Subset each binary to the declared `unicode-range` (Latin + Latin-Extended + numeric + ligatures + select symbols). Files: `public/assets/fonts/<family>/<weight>-<style>.woff2`. Acceptance: every shipped face combined per route stays under the spec's `Font Payload Budget` (per-route ≤ 180 KB, per-family-per-route ≤ 60 KB) — this is the binding constraint, enforced in Phase 5 by `measure-font-payload.mjs`. Individual WOFF2 byte-count SHOULD typically stay ≤ 60 KB after subsetting as a sanity guideline, but the route-level budget is what fails the build. Per Copilot review on PR #44 finding [F3].
...
- [ ] 4.1 Modify `src/layouts/BaseLayout.astro` to accept `Astro.props.preloadFonts?: string[]` (default: `["space-grotesk/500-normal"]`, using `"<family>/<weight>-<style>"` format that maps directly to the on-disk path per D7). Emit ONE `<link rel="preload" as="font" type="font/woff2" crossorigin>` per array entry in `<head>`, each resolving to `/assets/fonts/<family>/<weight>-<style>.woff2`. Files: `src/layouts/BaseLayout.astro`. Acceptance: built `.build/dist/index.html` contains 1-2 preload-links per route, matching the spec scenario "Preload count is bounded" (1 ≤ count ≤ 2). Per Copilot review on PR #44 findings [F7, F16].
- [ ] 4.2 Audit each `src/pages/**/*.astro` route's BaseLayout invocation. Marketing routes use the default `["space-grotesk/500-normal"]` (1 link). Long-form `/writing/*` routes pass `["fraunces/400-italic", "space-grotesk/500-normal"]` (2 links — Fraunces for body LCP, Space Grotesk for above-the-fold heading). Files: `src/pages/writing/[slug].astro` and any other long-form route. Acceptance: every route emits the right preload-links per its content type.
- [ ] 4.3 Add Playwright e2e test `tests/font-loading.spec.ts`: loads `/`, asserts `getComputedStyle(document.querySelector('h1')).fontFamily` resolves to "Space Grotesk" (or fallback if WOFF2 didn't load); asserts the preload-link element exists with the correct href + crossorigin attribute. Files: `tests/font-loading.spec.ts`. Acceptance: test passes on chromium + firefox + webkit. (Flat `tests/` layout per the live three-runner convention — `*.spec.ts` is Playwright.)

## 5. Phase 5 — `measure-font-payload.mjs` + budget gate

- [ ] 5.1 Build `scripts/measure-font-payload.mjs`: walk `.build/dist/**/*.html` (the configured output dir per `build.config.json`), parse `<link rel="preload" as="font">` + transitively reach `@font-face` URIs via referenced stylesheets, sum bytes per route + per-family-per-route, exit non-zero if any route > 180 KB OR any family > 60 KB. Output: JSON line per route. Files: `scripts/measure-font-payload.mjs`. Acceptance: against current `.build/dist`, exits 0 with summary; injecting a 200 KB WOFF2 reference flips to non-zero.
- [ ] 5.2 Add `node:test` tests: `tests/measure-font-payload.test.mjs`. Synthetic `.build/dist/` fixtures via mkdtemp. Files: new test. Acceptance: ≥ 6 tests covering happy path + per-route violation + per-family violation + JSON output shape. (Flat `tests/` layout per the live three-runner convention; see proposal.md Prerequisites.)
- [ ] 5.3 Add npm script `measure:font-payload` → `node scripts/measure-font-payload.mjs`. Files: `package.json`. Acceptance: invocable.
- [ ] 5.4 Build `scripts/verify-font-subset-coverage.mjs`: walks `src/content/**/*.mdx` + `src/pages/**/*.{astro,mdx}`, extracts every codepoint, fails non-zero if any falls outside the declared `unicode-range` from `theme.css` `@font-face` blocks. Output: file:line:codepoint per violation. Files: `scripts/verify-font-subset-coverage.mjs`. Acceptance: against current main, exits 0 (no out-of-subset codepoints today); injecting a Cyrillic character into a fixture page flips to non-zero. Per Copilot review on PR #44 finding [F10].
- [ ] 5.5 Add `node:test` tests: `tests/verify-font-subset-coverage.test.mjs`. Files: new test. Acceptance: ≥ 4 tests covering happy path + Cyrillic + Greek + emoji-PUA codepoints.
- [ ] 5.6 Add npm script `verify:font-subset-coverage` → `node scripts/verify-font-subset-coverage.mjs`. Files: `package.json`. Acceptance: invocable.

## 6. Phase 6 — Wire gates into postbuild

- [ ] 6.1 Update `package.json` `postbuild` to: `verify:prerequisites && verify:design-prerequisites && lint:tokens && verify-font-self-hosting && verify:font-metrics && measure:font-payload && verify:font-subset-coverage && sri && csp && lint:design && lint:design-md-uniqueness`. Files: `package.json`. Acceptance: chain runs in order; the THREE new gates (`verify:font-metrics`, `measure:font-payload`, `verify:font-subset-coverage`) execute every postbuild.
- [ ] 6.2 Update `tests/README.md` documenting the new test files. Files: `tests/README.md`. Acceptance: README mentions `font-loading.spec.ts`, `derive-font-metrics.test.mjs`, `verify-font-metrics.test.mjs`, `measure-font-payload.test.mjs`, `verify-font-subset-coverage.test.mjs` (flat `.mjs` layout per Prerequisites).
- [ ] 6.3 Add a "Typography" section to `AGENTS.md` describing the self-hosted WOFF2 contract: per-route preload-link via `BaseLayout.astro` `preloadFonts` prop, payload budgets (180 KB/route, 60 KB/family), `font-display: swap` policy, and the postbuild gates (`verify:font-metrics`, `measure:font-payload`). Files: `AGENTS.md`. Acceptance: an agent reading `AGENTS.md` knows where the font contract lives without consulting the proposal. Per Copilot review on PR #44 finding [F4] — was previously "remove mentions of fonts deferred" but AGENTS.md has no font section yet, so the task is now "ADD" not "REMOVE."
...
- [ ] 7.4 Push branch; confirm the existing CI gates (`playwright.yml` Test/Visual/Accessibility jobs + node:test/vitest umbrella via the test job's `npm run test:node` + `npm run test:vitest`) all succeed. Acceptance: PR shows green across the live workflow set. (Earlier draft text referenced a planned `quick-checks.yml` from a `modernize-unit-tests-with-vitest` change that was never authored; vitest is already wired into the existing playwright.yml + per-runner npm scripts, so no additional workflow file is required.)

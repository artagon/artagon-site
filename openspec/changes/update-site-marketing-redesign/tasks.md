...
- [x] 3.6 `ThemeToggle.astro` `<button>` Astro island with `aria-pressed` reflecting active theme + inline pre-paint script (CSP-hashed) reading `localStorage.theme`, validating against allow-list `['twilight', 'midnight']`, falling back to `twilight` on any other value, and applying `<html data-theme>` via `setAttribute` only (no `innerHTML`/`outerHTML`/string-concatenated attribute construction). Update `scripts/csp.mjs` to include the new inline-script SHA-256 AND to fail the build if any inline `<script>` SHA-256 in `dist/` is not present in the emitted `script-src` directive (orphan-hash detection); forbid `'unsafe-inline'`. → ✅ `ThemeToggle.astro` rewritten: `<button data-theme-toggle>` + moon/sun SVG icons (CSS-toggled by `aria-pressed`); `is:inline` aria-sync script (pre-paint, runs when component renders); bundled `<script>` for click handler; all mutations via `setAttribute` only; allowlist `['midnight','twilight']`; falls back to `'twilight'`. `BaseLayout.astro` pre-paint script updated: allowlist + `setAttribute` + fallback = `'twilight'`; `window.__setTheme` updated to validate + setAttribute + sync all `[data-theme-toggle]` buttons; static `data-theme` changed to `'twilight'`. `scripts/csp.mjs` orphan-hash detection: after building policy, verifies (a) `'unsafe-inline'` not in `script-src`, (b) every inline script SHA-256 is present in `script-src`; throws (fails build) on violation.

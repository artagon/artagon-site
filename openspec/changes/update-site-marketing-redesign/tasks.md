...
- [ ] 0.7 Adopt the conversion guidance prompt at `docs/guides/new-design-conversion.md` (v3.2, post-3-round adversarial review by Claude/Codex/Gemini). The prompt consolidates the rules an automated converter MUST follow to convert `new-design/extracted/*.{html,jsx,css}` into Astro without breaking the live site. Mandates from v3.2:
  ...
  - **`client:*` ban**: `@astrojs/react` is NOT installed; `client:load`/`client:visible`/`client:idle`/`client:only` are FORBIDDEN. Translate React hooks to CSS-only `:checked`/`:has()`/`<details>`/`popover` API or vanilla `<script>` blocks. If translation is impossible, file a separate change to add React BEFORE this conversion proceeds — do NOT mix.

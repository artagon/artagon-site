...
- [x] 4.5 Sample `src/content/pages/writing/welcome.mdx` (non-draft) to exercise the writing detail route. → ✅ shipped at `src/content/pages/writing/welcome.mdx`. Content adapted from `new-design/extracted/src/pages/post-trusted-identity.html` (h1 + lede + 3-section body — Why now / What Artagon builds / What's next). Frontmatter satisfies the writing schema: `published: 2026-05-06`, `draft: false`, `tags: [announcement, identity, vc, oidc]`. Also: Phase 4.1's content-collection topology fixed to use Astro 6 content layer (`glob` loader) so `pages` and `writing` can co-locate under `src/content/pages/` per spec while remaining distinct collections — `pages` glob is non-recursive (`*.{md,mdx}` at root), `writing` is rooted at `src/content/pages/writing/` with recursive glob. Vision page (existing collection consumer) still renders correctly; 10/10 chromium Playwright tests pass.

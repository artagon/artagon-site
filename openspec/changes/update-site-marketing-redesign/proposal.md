...
- **Affected Code (non-exhaustive):**
  - `src/content.config.ts` (Astro 6 layout — file at `src/content.config.ts` not nested under `src/content/config.ts` which was the Astro 5 path), `src/content/pages/*.mdx`, `src/content/pages/writing/*.mdx`, `src/content/authors/*.mdx`, `src/content/taglines.json`

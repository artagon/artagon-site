...
This Requirement governs ONLY the assertion shape (URLs, `assertMatrix`, severities, `minScore` thresholds, Core Web Vitals budgets). Output paths (`outputDir`, runtime cache directory) are governed by the `Lighthouse CI Ready Signal` Requirement under the `check-site-quality` capability (post `standardize-build-artifacts` archive). The sync generator at `scripts/sync-build-config.mjs` merges both at generation time: paths from `build-config`, assertions from `style-system`. `lighthouserc.json` is a generated file; direct edits are caught by the drift gate.

...
`artagon-site` is an Astro **6** static site (ESM, static output; the proposal was authored under Astro 5 — the 5 → 6 migration shipped mid-USMR per `package.json` `"astro": "6.2.1"`) with a strict "zero runtime JS by default" contract, SRI + CSP postbuild (`scripts/sri.mjs`, `scripts/csp.mjs`), Lighthouse CI, a Playwright test suite, Lychee link checks, and ast-grep (`sg`) as the primary linter (no ESLint). It has an existing `style-system` capability (post-archive of `fix-styling-refactor-gaps`) that owns theme tokens, `public/assets/theme.css`, and the `src/components/ui/` primitives; plus the `refactor-styling-architecture` change (extending that capability — was in-flight at proposal-authoring time; archived 2026-05-04 to `openspec/changes/archive/2026-05-04-refactor-styling-architecture/`). The existing `style-system` spec already references `data-theme="twilight"` in at least one scenario, so this change MODIFIES the relevant requirement (does not introduce twilight) and ADDS `midnight` alongside.
...
`site-structured-data` capability emits Organization + WebSite sitewide (Pre-pt407 said "NO `potentialAction` SearchAction while `/search` is `noindex` — would generate Search Console warnings". Per USMR pt152 the SearchAction DID ship targeting `${SITE}/search?q={search_term_string}` — `/search` exists at `src/pages/search/index.astro` and serves real DocSearch UI; the route remains `noindex` but the canonical-fidelity decision shipped the SearchAction anyway because the discoverability benefit outweighs the SC warning class. The live-spec contract at `openspec/specs/manage-site-links/spec.md` "Omit SearchAction" Requirement has an "unless a stable non-redirecting search endpoint is available" clause that pt152 reconciled to — see `manage-site-links/spec.md` Purpose KNOWN SPEC DRIFT note added in pt395), Article + BreadcrumbList on writing posts (Article includes `publisher` resolved from sitewide Organization; missing `cover` emits a non-fatal Top Stories warning), and DefinedTerm per standards entry (entity/Knowledge Graph signal, not a triggered SERP rich-result feature).

JSON-LD via the `safeJsonLd` helper at `src/lib/charset.ts` (pre-pt407 cited `JsonLd.astro` for the planned dedicated component; the implementation uses inline `set:html={safeJsonLd(...)}` instead — `JsonLd.astro` does not exist, the safeJsonLd helper is the canonical SSoT. The pre-pt407 "no `set:html`" claim was inverted: `set:html` IS used for JSON-LD specifically, with the ast-grep `no-set-html-directive` rule's per-line allow-list pragma covering the safeJsonLd output sites; this is the same pattern documented in pt405 fix to `specs/site-structured-data/spec.md:47`). The helper uses `JSON.stringify` then Astro's safe interpolation inside `<script type="application/ld+json">`. Before interpolation, the payload escapes `<` as `<` (also `>` and `&`) so user-controlled MDX strings cannot terminate the script element via `</script>` injection. `validate-structured-data.mjs` asserts no raw `</script>` substring inside any ld+json block in `dist/` AND enforces an aggregate ld+json size ceiling of 8 KB uncompressed per route. `JSON.stringify` runs without indent argument.

### 14. Sitemap + noindex + robots — single source

`site-indexation` capability regenerates `sitemap.xml` on build with `<lastmod>` bound to MDX `updated`/`published` frontmatter (NOT CI checkout mtime — `@astrojs/sitemap`'s default would tell Google every page updated on every deploy). **Planned**: the list of non-indexable routes will live in `src/lib/indexation.ts` `NOINDEX_ROUTES` as a typed `as const` array consumed by sitemap filter, BaseLayout `indexable` prop, robots.txt generator, and `validate-indexation.mjs` — single source prevents drift. **Today (pre-pt408 said "lives in"; corrected to "will live in" — Phase 10 tasks 10.0 and 10.7 still unchecked, indexation.ts doesn't exist)**: NOINDEX_ROUTES is inline in `astro.config.ts:32-43` (pt146; same content as the planned typed array — 10 routes: /404, /search, /privacy, /how, /security, /status, /docs, /play, /console, /developers); `validate-indexation.mjs` is also unshipped per Phase 10.7. Excluded routes emit `<meta name="robots" content="noindex, nofollow">` already today via the per-page `robots` prop on BaseLayout. `robots.txt` managed under this capability. `/_drafts/` filter is in `astro.config.ts` (renamed from `astro.config.mjs` mid-USMR per pt342 archaeology). `_redirects` destinations MUST be same-origin (begin with `/`); validator rejects `://` and `//`.

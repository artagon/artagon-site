...
7. **Focus indicators**: scenario-picker dots have `:focus-visible { outline: 2px solid var(--accent); }` (per `src/components/TrustChainIsland.css:120-124`; `--accent` is the canonical token, `--brand-teal` is a retained alias per pt86). Stage rows, decision card, on-ramp CTAs, and writing-strip cards do NOT — they fall back to browser-default focus rings, which can be invisible against tinted backgrounds (`var(--bg-1)` blue-gray vs. Chrome's default thin black ring; `--bg-alt` was the pre-pt170 alias that was pruned, replaced by `--bg-1`).

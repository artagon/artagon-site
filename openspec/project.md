...
- **Precedence chain (canonical):** `openspec/specs/*` govern behavior; `DESIGN.md` (repo root, governed by `adopt-design-md-format`) governs visual presentation; implementation traces to both. On conflict, the spec wins and `DESIGN.md` is updated in the same change.

...
This capability defines the contracts that govern how GitHub
Copilot coding-agent sessions are bootstrapped against this
repository. The capability ships:

- A `copilot-setup-steps` workflow at
  `.github/workflows/copilot-setup-steps.yml` that runs `npm ci`
  to prepare deps for any Copilot agent session.
- A repo-root context file `COPILOT.md` that orients agents to
  the OpenSpec workflow.
- Two `.github/copilot-{instructions,review-instructions}.md`
  files that describe spec-compliance review expectations and
  the structured review format.

The capability was created by archiving the
`add-copilot-integration` change
(`openspec/changes/archive/2025-12-29-add-copilot-integration/`).

**KNOWN SPEC DRIFT (pt238 archaeology)**: Requirements below
mandate that `COPILOT.md` and the `.github/copilot-*-
instructions.md` files reference `openspec/AGENTS.md`, but that
file was deleted in commit `f1cc5df` when its content migrated
into the per-skill `opsx:*` scaffolds at `.agents/skills/`. The
live `COPILOT.md` correctly references the surviving
`openspec/project.md` + `openspec/contributing.md` instead.
The Requirements below need a follow-up amendment (separate
OpenSpec proposal) to point at the live OpenSpec context files
rather than the deleted aggregate; pt282 is doc-scope (Purpose
backfill) only and does NOT modify the Requirements themselves
per OpenSpec discipline (Requirements changes go through
